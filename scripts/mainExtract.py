...
def getData(insulin_df, cgm_df):
    return getInsulinData(insulin_df), getCGMData(cgm_df)

def getInsulinData(insulin_df):
...
def mealInterval(cgm_data_point, insulin_meal_point):
    cgm_df = cgm_data_point.copy()
    insulinDF = insulin_meal_point.copy()

    cgm_df = cgm_df.loc[cgm_df['Sensor Glucose (mg/dL)'].notna()]
    cgm_df.set_index(['Date_Time'],inplace=True)
    cgm_df = cgm_df.sort_index().reset_index()

    insulinDF.set_index(["Date_Time_caller"],inplace=True)
    insulinDF = insulinDF.sort_index().reset_index()
    
    result = pd.merge_asof(insulinDF, cgm_df,left_on='Date_Time_caller',right_on='Date_Time',direction="forward")
    
    return result

def getMealData(cgmDF, insDF):
    insTime = getMealTime(insDF)
    result = mealInterval(cgmDF, insTime)
    return result

def getSensorTimenterval(df, val):
...
cgmDF, insDF = getData('CGMData.csv','InsulinData.csv')
meal_data = getMealData(cgmDF, insDF)

def getRootMeanSquare(row):
    root_mean_square = 0
    for p in range(0, len(row) - 1):
        root_mean_square = root_mean_square + np.square(row[p])
    return np.sqrt(root_mean_square / len(row))

def getAbsoluteValueMean(row):
    mean_val = 0
    for p in range(0, len(row) - 1):
...
def mealFeatureMatrix(dataFrameMeal, insulinDF):
    ...
    for index, x in dataFrameMeal.iterrows():
        ...
        isCorrect, meal = getSensorTimenterval(meal, 30)
